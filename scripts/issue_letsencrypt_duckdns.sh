...
work_dir=$(mktemp -d /tmp/local-ai-letsencrypt.XXXXXX)
...
if [[ -n "$existing_certificate_arn" ]]; then
  ...
  import_arguments+=(
    --tags "Key=Project,Value=local-ai-assistant" "Key=ManagedBy,Value=letsencrypt-duckdns"
  )
fi

certificate_arn=$(aws acm import-certificate "${import_arguments[@]}" \
